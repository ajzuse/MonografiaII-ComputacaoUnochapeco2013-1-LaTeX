...
\subsubsection{XML}
Segundo a  W3C (2012), a Linguagem Extensível de Marcação(XML) é uma linguagem de marcação baseada em texto para representar estruturas de informação. Foi derivada de um padrão antigo chamado \emph{Standard Generalized Markup Language} (SGML), tornando-o mais apropriado para a Web.

O XML apresenta muitas vantagens sobre outros formatos, sendo que cada tag é definida conforme a necessidade da aplicação. Devido a cada tag poder assumir qualquer nome, o documento se torna auto-descritivo, possibilitando a interpretação do documento sem a necessidade de manuais \cite{W3C}.
